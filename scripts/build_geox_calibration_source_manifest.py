...
def build(output=DEFAULT):
 ...
 for c in sorted(cases,key=lambda x:x['case_id']):
  ...
  if r['readout_id']!=r['readout_id'] or r['experiment_id']!=fid: raise ValueError('readout identity mismatch')
  ...
  if any(rec['authorization_flags'].values()): raise ValueError('authorization')
  records.append(rec)
