...
The generated view is the deterministic completion record for the canonical
execution state. The active task contract specifies the focused validation
policy and the restriction against analytical, product, runtime, certification,
capability, MIP, MMM, and successor-task changes. Historical/rejected branches
are immutable evidence only and must not provide executable ancestry.
