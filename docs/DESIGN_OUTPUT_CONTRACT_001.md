...
| `requested_n_strata` / `realized_n_strata` | Stratified (DES-004): via `StratifiedRandomization.last_stratification_metadata` when `stratification_policy != legacy` |
...
**Estimator readout integration (2026-06-18):** [`ESTIMATOR_READOUT_GUARDRAIL_INTEGRATION_001.md`](ESTIMATOR_READOUT_GUARDRAIL_INTEGRATION_001.md) provides `build_estimator_readout()` / `run_governed_analysis()` for governed `ReadoutEvidence`. [`DOWNSTREAM_READOUT_AUTHORIZATION_GATEWAY_001.md`](DOWNSTREAM_READOUT_AUTHORIZATION_GATEWAY_001.md) enforces fail-closed downstream authorization (all production roles BLOCKED).
