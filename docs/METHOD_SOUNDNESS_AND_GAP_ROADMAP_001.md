...
**Active method lane (2026-06-03):** `METHOD_ACCURACY_COMPATIBILITY_REFOCUS_AUDIT_001` → … → `SYNTHETIC_DID_METHOD_SCOUT_AND_SUITABILITY_001` ✅ → `BAYESIAN_TBR_AND_TBR_RETIREMENT_BOUNDARY_AUDIT_001` ✅ → **`TROP_RESEARCH_ONLY_BOUNDARY_AUDIT_001`** (immediate next). Downstream schema/ingestion/decisioning paused.

**BAYESIAN_TBR_AND_TBR_RETIREMENT_BOUNDARY_AUDIT_001:** 48-row TBR/Bayesian TBR boundary audit (`failed_scenarios: []`). Posterior intervals not causal CIs; classic aggregate overclaim blocked. See [`BAYESIAN_TBR_AND_TBR_RETIREMENT_BOUNDARY_AUDIT_001`](track_d/BAYESIAN_TBR_AND_TBR_RETIREMENT_BOUNDARY_AUDIT_001_REPORT.md).
