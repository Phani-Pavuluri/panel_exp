...
**Active lanes (2026-06-03):** SCM closeout: `SCM_PRODUCTION_CANDIDATE_CLOSEOUT_AND_METHOD_PORTFOLIO_HANDOFF_001` (immediate). **Portfolio planner roadmap:** `EXPERIMENT_PORTFOLIO_PLANNER_AGENT_ROADMAP_001` ✅; first planner implementation lane: `EXPERIMENT_PORTFOLIO_INTAKE_CONTRACT_001`. **Primary method focus after planner lanes:** `AUGSYNTH_ASCM_REMEDIATION_IMPLEMENTATION_001`. SCM remains governed reference candidate per `METHOD_PORTFOLIO_PRIORITIZATION_CHECKPOINT_001`. Design selection precedes estimator selection. Downstream schema/ingestion/decisioning paused.
