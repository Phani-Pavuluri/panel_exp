...
**Active method lane (2026-06-03):** `METHOD_ACCURACY_COMPATIBILITY_REFOCUS_AUDIT_001` → … → `SCM_AUGSYNTH_STATISTIC_ADAPTER_CONTRACT_001` ✅ → `ROADMAP_INFERENCE_AND_METHOD_GAP_CONTROL_REFOCUS_001` ✅ → `ESTIMATOR_DESIGN_INFERENCE_SUITABILITY_MATRIX_001` ✅ → **`METHOD_GAP_COVERAGE_AND_LITERATURE_ALIGNMENT_AUDIT_001`** (immediate next) → `OBSERVED_PANEL_DIAGNOSTIC_REQUIREMENTS_001` → `SIMULATION_DGP_COVERAGE_PLAN_001` → `METHOD_FAILURE_MODE_REGISTRY_001`. Downstream schema/ingestion/decisioning paused.

**ROADMAP_INFERENCE_AND_METHOD_GAP_CONTROL_REFOCUS_001:** Placebo/randomization is one inference family, not the full inference layer. No estimator receives universal default inference. Observed-data conditions, design validity, estimator/inference suitability, simulation coverage, literature alignment, failure modes, and repair/replace/retire decisions are first-class method-control layers. `DESIGN_ASSIGNMENT_GENERATOR_STRESS_TESTS_001` remains important but follows suitability/gap-control unless reprioritized.

**ESTIMATOR_DESIGN_INFERENCE_SUITABILITY_MATRIX_001:** Completed 50-row suitability matrix (`failed_scenarios: []`; all 8 estimator families · 12 design families · 17 inference families). Placebo/randomization is not the whole inference layer. See [`ESTIMATOR_DESIGN_INFERENCE_SUITABILITY_MATRIX_001`](track_d/ESTIMATOR_DESIGN_INFERENCE_SUITABILITY_MATRIX_001_REPORT.md). No production p-values, causal CIs, TrustReport, CalibrationSignal, MMM, LLM, production decisioning, live API, scheduler, or budget optimization.
