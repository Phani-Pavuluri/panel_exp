...
**Active method lane (2026-06-03):** `DESIGN_AWARE_ASSIGNMENT_GENERATORS_001` → `MULTITREATED_TREATED_SET_PLACEBO_FRAMEWORK_001` → `SCM_PLACEBO_GOVERNED_SEMANTICS_001` → `METHOD_SPECIFIC_RANDOMIZATION_INFERENCE_VALIDATION_001` → `SCM_TREATED_SET_PLACEBO_INTEGRATION_001` → `STUDENTIZED_PLACEBO_RANK_INFERENCE_001` → `SCM_STUDENTIZED_TREATED_SET_PLACEBO_INTEGRATION_001` → `MULTICELL_SHARED_CONTROL_MULTIPLICITY_001` → `STRATIFIED_POOLED_ESTIMAND_CONTRACT_001` → `AUGSYNTH_POINT_RANDOMIZATION_INTEGRATION_001` ✅ → `METHOD_READINESS_MATRIX_V2_001` per [`INFERENCE_REPLACEMENT_SCOUT_001`](audits/INFERENCE_REPLACEMENT_SCOUT_001.md).
