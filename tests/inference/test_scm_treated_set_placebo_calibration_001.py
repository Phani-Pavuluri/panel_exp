"""Tests for scm_treated_set_placebo_calibration inference module."""
...
def _spec(**kwargs) -> SCMTreatedSetNullSimulationSpec:
    defaults = dict(
...
def test_valid_spec_passes() -> None:
    ok, reasons = validate_scm_treated_set_null_simulation_spec(_spec())
    assert ok
    assert not reasons


def test_invalid_unit_count_blocked() -> None:
    ok, _ = validate_scm_treated_set_null_simulation_spec(_spec(num_units=2))
    assert not ok


def test_invalid_treated_count_blocked() -> None:
    ok, _ = validate_scm_treated_set_null_simulation_spec(_spec(num_treated=0))
    assert not ok


def test_treated_ge_units_blocked() -> None:
    ok, _ = validate_scm_treated_set_null_simulation_spec(_spec(num_treated=12))
    assert not ok


def test_invalid_periods_blocked() -> None:
    ok, _ = validate_scm_treated_set_null_simulation_spec(_spec(num_pre_periods=1))
    assert not ok


def test_invalid_alpha_blocked() -> None:
    ok, _ = validate_scm_treated_set_null_simulation_spec(_spec(alpha_levels=(0.0,)))
    assert not ok


def test_too_few_replications_blocked() -> None:
    ok, _ = validate_scm_treated_set_null_simulation_spec(
...
def test_iid_simulation_runs() -> None:
    result = run_scm_treated_set_null_calibration(_spec(dgp_kind=SCMNullDGPKind.IID_NORMAL))
    assert len(result.replication_results) >= 20
    assert result.verdict != SCMCalibrationVerdict.INVALID_CALIBRATION_SPEC


def test_unit_fixed_effect_runs() -> None:
    result = run_scm_treated_set_null_calibration(_spec(dgp_kind=SCMNullDGPKind.UNIT_FIXED_EFFECT))
    assert len(result.replication_results) >= 20


def test_unit_time_fixed_effect_runs() -> None:
    result = run_scm_treated_set_null_calibration(
        _spec(dgp_kind=SCMNullDGPKind.UNIT_AND_TIME_FIXED_EFFECT)
    )
    assert len(result.replication_results) >= 20


def test_heteroskedastic_runs() -> None:
    result = run_scm_treated_set_null_calibration(_spec(dgp_kind=SCMNullDGPKind.HETEROSKEDASTIC))
    assert len(result.replication_results) >= 20


def test_donor_matched_latent_factor_runs() -> None:
    result = run_scm_treated_set_null_calibration(
        _spec(dgp_kind=SCMNullDGPKind.DONOR_MATCHED_LATENT_FACTOR)
    )
    assert len(result.replication_results) >= 20
...
def test_complete_randomized_covered() -> None:
    result = run_scm_treated_set_null_calibration(
        _spec(assignment_family=SCMAssignmentFamily.COMPLETE_RANDOMIZED_SET)
    )
    assert len(result.replication_results) >= 20
...
def test_scm_style_effect_mode() -> None:
    result = run_scm_treated_set_null_calibration(
        _spec(statistic_mode=SCMStatisticMode.SCM_STYLE_EFFECT)
    )
    assert result.replication_results


def test_scm_style_studentized_mode() -> None:
    result = run_scm_treated_set_null_calibration(
        _spec(statistic_mode=SCMStatisticMode.SCM_STYLE_STUDENTIZED_EFFECT)
    )
    assert result.replication_results


def test_diff_in_means_baseline_mode() -> None:
    result = run_scm_treated_set_null_calibration(
        _spec(statistic_mode=SCMStatisticMode.SIMPLE_DIFF_IN_MEANS_BASELINE)
    )
    assert result.replication_results


def test_compute_scm_style_statistic() -> None:
    outcomes = np.random.default_rng(0).normal(size=(4, 6))
    panel = OutcomePanel(outcomes=outcomes, unit_ids=("u0", "u1", "u2", "u3"))
    pre = tuple(f"p{i}" for i in range(4))
    post = ("p4", "p5")
    stat = compute_scm_style_treated_set_statistic(
        panel, ("u0", "u1"), pre, post, SCMStatisticMode.SCM_STYLE_EFFECT
    )
    assert np.isfinite(stat)


def test_tail_fraction_bounded() -> None:
    result = run_scm_treated_set_null_calibration(_spec())
...
def test_seed_reproducibility() -> None:
    r1 = run_scm_treated_set_null_calibration(_spec(seed=5))
    r2 = run_scm_treated_set_null_calibration(_spec(seed=5))
    assert (
...
def test_different_seed_changes_path() -> None:
    r1 = run_scm_treated_set_null_calibration(_spec(seed=5))
    r2 = run_scm_treated_set_null_calibration(_spec(seed=6))
    assert (
        r1.replication_results[0].empirical_tail_fraction
        != r2.replication_results[0].empirical_tail_fraction
    )
...
def test_verdict_assigned() -> None:
    result = run_scm_treated_set_null_calibration(_spec())
    assert result.verdict in SCMCalibrationVerdict


def test_grid_summary_valid() -> None:
    grid = build_default_scm_treated_set_null_calibration_grid()
    small = tuple(
        SCMTreatedSetNullSimulationSpec(
            dgp_kind=s.dgp_kind,
    ...
    results = run_scm_treated_set_null_calibration_grid(small)
    summary = summarize_scm_treated_set_null_calibration_grid(results)
    assert summary["grid_result_count"] == 3
    assert "comparison_summary" in summary
    assert "scm_style_effect" in summary["comparison_summary"]["max_type_i_excess_by_statistic_mode"]


def test_comparison_identifies_max_mode() -> None:
    grid = build_default_scm_treated_set_null_calibration_grid()
    small = tuple(
        SCMTreatedSetNullSimulationSpec(
            dgp_kind=s.dgp_kind,
    ...
    results = run_scm_treated_set_null_calibration_grid(small)
    summary = summarize_scm_treated_set_null_calibration_grid(results)
    assert summary["comparison_summary"]["max_type_i_excess_mode"] is not None
