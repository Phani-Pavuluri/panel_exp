...
from panel_exp.validation.track_d_d5_stat_tbr_agg_001 import (
    D5StatTbrAgg001Config,
    REQUIRED_WORLD_IDS,
    build_d5_stat_tbr_agg_001,
    write_artifact,
)

_REPO = Path(__file__).resolve().parents[2]
ARTIFACT = _REPO / "docs/track_d/archives/D5_STAT_TBR_AGG_001_results.json"
REPORT = _REPO / "docs/track_d/D5_STAT_TBR_AGG_001_REPORT.md"
...
def _fast_cfg() -> D5StatTbrAgg001Config:
    return D5StatTbrAgg001Config(n_replicates=4)


class TestD5StatTbrAgg001:
    def test_build_deterministic_excluding_timestamp(self) -> None:
        cfg = _fast_cfg()
        assert _strip_timestamp(build_d5_stat_tbr_agg_001(cfg)) == _strip_timestamp(
            build_d5_stat_tbr_agg_001(cfg)
        )

    def test_required_worlds_exist(self) -> None:
        payload = build_d5_stat_tbr_agg_001(_fast_cfg())
    ...
    def test_unit_panel_generalization_forbidden(self) -> None:
        payload = build_d5_stat_tbr_agg_001(_fast_cfg())
        assert payload["forbidden_flags"]["unit_panel_generalization_allowed"] is False
        assert payload["summary"]["unit_panel_generalization_allowed"] is False
    ...
    def test_runtime_modest(self) -> None:
        ...
        build_d5_stat_tbr_agg_001(_fast_cfg())
        assert time.perf_counter() - t0 < 60.0

    def test_committed_artifact_matches_build(self) -> None:
        if not ARTIFACT.is_file():
            pytest.skip("Run generator")
        cfg = D5StatTbrAgg001Config(n_replicates=15)
        loaded = _strip_timestamp(json.loads(ARTIFACT.read_text(encoding="utf-8")))
        built = _strip_timestamp(build_d5_stat_tbr_agg_001(cfg))
        assert loaded == built


def test_write_artifact(tmp_path: Path) -> None:
    assert write_artifact(tmp_path / "tbr_agg.json", cfg=_fast_cfg()).is_file()
