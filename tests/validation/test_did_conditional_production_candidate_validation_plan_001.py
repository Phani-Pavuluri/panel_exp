"""Tests for DID_CONDITIONAL_PRODUCTION_CANDIDATE_VALIDATION_PLAN_001."""
...
from panel_exp.validation.did_conditional_production_candidate_validation_plan_001 import (
    MIN_VALIDATION_ROW_COUNT,
    RECOMMENDED_NEXT_ARTIFACTS,
    REQUIRED_ELIGIBILITY_TYPES,
    REQUIRED_STATUSES,
    ...
    _AUTH_FLAGS,
    _BOUNDARY_FLAGS,
    build_did_conditional_production_candidate_validation_plan,
    build_scenarios,
    filter_did_conditional_production_candidate_validation_plan,
    run_validation,
    summarize_did_conditional_production_candidate_validation_plan,
    validate_did_conditional_production_candidate_validation_plan,
)
...
_REPORT = _REPO / "docs/track_d/DID_CONDITIONAL_PRODUCTION_CANDIDATE_VALIDATION_PLAN_001_REPORT.md"
...
def test_all_areas_statuses_and_eligibility_types_represented() -> None:
    rows = build_did_conditional_production_candidate_validation_plan()
    ...
    assert validation["all_required_eligibility_types_covered"]
...
def test_filter_helpers() -> None:
    rows = build_did_conditional_production_candidate_validation_plan()
    parallel = filter_did_conditional_production_candidate_validation_plan(
        rows, validation_area=ValidationArea.PARALLEL_TREND
    )
    assert parallel
    blocked = filter_did_conditional_production_candidate_validation_plan(
        rows, current_status=ValidationStatus.BLOCKED
    )
    assert blocked
    adapter = filter_did_conditional_production_candidate_validation_plan(
        rows, requires_adapter=True
    )
    assert adapter
    null_cal = filter_did_conditional_production_candidate_validation_plan(
        rows, requires_null_calibration=True
    )
    assert null_cal
    trend = filter_did_conditional_production_candidate_validation_plan(
        rows, eligibility_type=EligibilityType.TREND_ELIGIBILITY
    )
    assert trend
...
def test_summary_count_consistency() -> None:
    ...
    assert summary["eligibility_type_counts"] == validation["eligibility_type_counts"]
...
def test_summary_json_and_report_exist() -> None:
    ...
    assert data["artifact_id"] == "DID_CONDITIONAL_PRODUCTION_CANDIDATE_VALIDATION_PLAN_001"
    ...
    for etype in REQUIRED_ELIGIBILITY_TYPES:
        assert data["eligibility_type_counts"].get(etype.value, 0) > 0


def test_report_states_no_authorization() -> None:
    text = _REPORT.read_text(encoding="utf-8")
    assert "does not authorize DID production inference" in text
