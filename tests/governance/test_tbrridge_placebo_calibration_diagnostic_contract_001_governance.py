"""Governance registry tests for TBRRIDGE_PLACEBO_CALIBRATION_DIAGNOSTIC_CONTRACT_001."""
...
def test_summary_complete() -> None:
    data = _load_summary()
    assert data["artifact_id"] == "TBRRIDGE_PLACEBO_CALIBRATION_DIAGNOSTIC_CONTRACT_001"
    assert data["placebo_calibration_diagnostic_contract_defined"] is True
...
def test_governance_registry_references_contract() -> None:
    ...
    assert "INV-TBRRIDGE-PLACEBO-CALIBRATION-DIAGNOSTIC-CONTRACT-001" in inv_ids
    lane_ids = {lane["lane_id"] for lane in reg["roadmap_lane_bindings"]}
    assert "TBRRIDGE-PLACEBO-CALIBRATION-DIAGNOSTIC-CONTRACT-001" in lane_ids
