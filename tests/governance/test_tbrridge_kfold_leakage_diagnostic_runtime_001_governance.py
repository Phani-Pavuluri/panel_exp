"""Governance registry tests for TBRRIDGE_KFOLD_LEAKAGE_DIAGNOSTIC_RUNTIME_001."""
...
def test_summary_runtime_complete() -> None:
    data = _load_summary()
    assert data["artifact_id"] == "TBRRIDGE_KFOLD_LEAKAGE_DIAGNOSTIC_RUNTIME_001"
...
def test_governance_registry_references_runtime() -> None:
    ...
    assert "INV-TBRRIDGE-KFOLD-LEAKAGE-DIAGNOSTIC-RUNTIME-001" in inv_ids
    lane_ids = {lane["lane_id"] for lane in reg["roadmap_lane_bindings"]}
    assert "TBRRIDGE-KFOLD-LEAKAGE-DIAGNOSTIC-RUNTIME-001" in lane_ids
