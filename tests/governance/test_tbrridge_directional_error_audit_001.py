"""Governance tests for TBRRIDGE_DIRECTIONAL_ERROR_AUDIT_001."""
...
_AUDIT = _REPO / "docs/track_d/TBRRIDGE_DIRECTIONAL_ERROR_AUDIT_001.md"
_SUMMARY = _REPO / "docs/track_d/archives/TBRRIDGE_DIRECTIONAL_ERROR_AUDIT_001_summary.json"
_OPEN_INVESTIGATIONS = _REPO / "docs/governance/OPEN_INVESTIGATIONS_001.json"

_DIRECTIONAL_FIXTURE_FAMILIES = (
    "positive-effect synthetic injection controls",
    "negative-effect synthetic injection controls",
    "near-zero signed-effect controls",
    "sign-flip stress controls",
    "heterogeneous-effect signed controls",
    "weak-preperiod-fit signed controls",
    "sparse-metric signed controls",
    "high-noise signed controls",
    "seasonal signed controls",
    "outlier-contaminated signed controls",
    "donor-pool-shift signed controls",
    "fold-geometry signed controls",
)

_DIRECTIONAL_METRICS = (
    "directional_error_rate_diagnostic",
    "positive_to_negative_error_rate_diagnostic",
    "negative_to_positive_error_rate_diagnostic",
    "near_zero_sign_instability_diagnostic",
    "sign_confidence_overstatement_rate_diagnostic",
    "signed_interval_exclusion_diagnostic",
    "directional_stability_by_fold",
    "directional_stability_by_donor_pool",
    "directional_stability_by_regularization",
    "directional_stability_by_metric_scale",
    "directional_stability_by_noise_regime",
)
...
_ALLOWED_LANGUAGE = (
    "directional-error diagnostic",
    "wrong-sign behavior audit",
    "signed-effect stress case",
    "review-only directional diagnostic",
    "sign instability diagnostic",
...
_FORBIDDEN_TRUE_FLAGS = (
    "directional_error_evidence_generated",
    "directional_error_rate_computed",
    "sign_accuracy_validated",
    "p_value_calibrated",
...
_POSITIVE_FLAGS = (
    "tbrridge_directional_error_audit_completed",
    "directional_error_gap_documented",
    "directional_fixture_requirements_defined",
    "directional_diagnostic_metrics_defined",
    "false_positive_distinction_defined",
...
def test_audit_doc_exists() -> None:
    ...
    assert (
        "tbrridge_directional_error_audited_no_directional_error_computation_or_authorization"
        in text
    )
...
def test_summary_json_exists_and_valid() -> None:
    data = _load_summary()
    assert data["artifact_id"] == "TBRRIDGE_DIRECTIONAL_ERROR_AUDIT_001"
...
def test_directional_error_evidence_gap_documented() -> None:
    text = _AUDIT.read_text(encoding="utf-8")
    assert "Directional-error evidence gap" in text
    data = _load_summary()
    assert data["directional_error_gap_documented"] is True


def test_directional_fixture_requirements_documented() -> None:
    text = _AUDIT.read_text(encoding="utf-8")
    assert "Required directional fixture families" in text
    for fixture in _DIRECTIONAL_FIXTURE_FAMILIES:
        assert fixture in text
    data = _load_summary()
    assert data["directional_fixture_requirements_defined"] is True


def test_directional_diagnostic_metrics_documented() -> None:
    text = _AUDIT.read_text(encoding="utf-8")
    assert "Directional diagnostic metrics to be reported later" in text
    for metric in _DIRECTIONAL_METRICS:
        assert metric in text
    data = _load_summary()
    assert data["directional_diagnostic_metrics_defined"] is True


def test_false_positive_distinction_documented() -> None:
    text = _AUDIT.read_text(encoding="utf-8")
    assert "Distinction from false-positive behavior" in text
    assert "false positive" in text.lower()
    data = _load_summary()
    assert data["false_positive_distinction_defined"] is True
...
def test_runtime_packet_integration_documented() -> None:
    ...
    assert "directional_error_report" in text
...
def test_recommended_next_artifact() -> None:
    data = _load_summary()
    assert data["recommended_next_artifact"] == "TBRRIDGE_POSITIVE_CONTROL_RECOVERY_AUDIT_001"
    text = _AUDIT.read_text(encoding="utf-8")
...
def test_governance_registry_references_artifact() -> None:
    ...
    assert "INV-TBRRIDGE-DIRECTIONAL-ERROR-AUDIT-001" in inv_ids
    lane_ids = {lane["lane_id"] for lane in reg["roadmap_lane_bindings"]}
    assert "TBRRIDGE-DIRECTIONAL-ERROR-AUDIT-001" in lane_ids
    inv = next(
        i for i in reg["investigations"]
        if i["investigation_id"] == "INV-TBRRIDGE-DIRECTIONAL-ERROR-AUDIT-001"
    )
    assert inv["status"] == "RESOLVED"
    assert inv["resolution_artifact"] == "TBRRIDGE_DIRECTIONAL_ERROR_AUDIT_001"
