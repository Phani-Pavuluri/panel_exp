"""Governance registry tests for TBRRIDGE_UNCERTAINTY_CANDIDATE_REVIEW_CONTRACT_001."""
...
def test_summary_complete() -> None:
    data = _load_summary()
    assert data["artifact_id"] == "TBRRIDGE_UNCERTAINTY_CANDIDATE_REVIEW_CONTRACT_001"
    assert data["uncertainty_candidate_review_contract_defined"] is True
    assert data["uncertainty_candidate_approved"] is False
    assert data["recommended_next_artifact"] == "TBRRIDGE_UNCERTAINTY_CANDIDATE_REVIEW_RUNTIME_001"
...
def test_governance_registry_references_contract() -> None:
    ...
    assert "INV-TBRRIDGE-UNCERTAINTY-CANDIDATE-REVIEW-CONTRACT-001" in inv_ids
    lane_ids = {lane["lane_id"] for lane in reg["roadmap_lane_bindings"]}
    assert "TBRRIDGE-UNCERTAINTY-CANDIDATE-REVIEW-CONTRACT-001" in lane_ids


def test_report_documents_contract() -> None:
    text = _REPORT.read_text(encoding="utf-8")
    assert "uncertainty_candidate_review_contract_defined" in text
