"""Track E triangulation evaluation (E3/E5 production path).

Evaluates ``TriangulationProfile`` shapes, applies E5 CalibrationSignal
eligibility policy, and checks E4 forbidden-action constraints.
"""
...
def _rows(profile: Mapping[str, Any]) -> list[dict[str, Any]]:
...
def _non_comparable_estimand(rows: list[dict[str, Any]], declared: str) -> bool:
    ...
    return "non_comparable_estimand" in groups or any(
        r.get("estimand_id")
        and r.get("estimand_id") != declared
        and r.get("comparability_group") == "non_comparable_estimand"
        for r in rows
    )


def _detect_cell_level_conflict(profile: Mapping[str, Any], primaries: list[dict[str, Any]]) -> bool:
...
def _per_cell_dispositions(
    primaries: list[dict[str, Any]], *, conflict: bool = False
) -> dict[str, str]:
    out: dict[str, str] = {}
    for r in primaries:
...
def evaluate_triangulation(profile: Mapping[str, Any]) -> TriangulationOutcome:
    """Evaluate E3 profile against E4/E5 disposition priority."""
    rows = _rows(profile)
...
def evaluate_triangulation_fixture(
    fixture: Mapping[str, Any],
) -> TriangulationOutcome:
    """Evaluate an E4 JSON fixture (profile + forbidden_actions)."""
...
DISPOSITION_ALIGNMENT: dict[str, str] = {
    "directional_support_with_caveats": "aligned",
    "restricted_method_positive_but_primary_null_compatible": "aligned",
    "method_conflict_warning": "divergent",
    "non_comparable_estimand": "incompatible",
    "cell_level_conflict": "divergent",
    "blocked_by_geometry": "not_assessable",
    "planning_diagnostic_only": "aligned",
    "characterization_required": "not_assessable",
    "missing_uncertainty_warning": "incompatible",
    "stale_downweight_or_exclude": "not_assessable",
}


def trust_verdicts_from_triangulation(
    outcome: TriangulationOutcome,
    claim_type: str,
) -> tuple[str, str]:
    """Map triangulation outcome to (alignment_verdict, trust_outcome) with claim caps."""
    disp = outcome.trust_report_disposition
    alignment = DISPOSITION_ALIGNMENT.get(disp, "not_assessable")
    trust: str = outcome.trust_outcome_hint

    if claim_type in LIFT_CLAIM_TYPES:
        if trust == "supported":
            trust = "inconclusive"
        if disp == "method_conflict_warning":
            trust = "divergent"
        elif disp == "directional_support_with_caveats":
            trust = "inconclusive"
        elif disp in (
            "restricted_method_positive_but_primary_null_compatible",
            "planning_diagnostic_only",
            ...
            trust = "inconclusive"
        elif disp in ("missing_uncertainty_warning", "non_comparable_estimand"):
            trust = "unsupported"
        elif disp in (
            "blocked_by_geometry",
            "characterization_required",
    ...
    if claim_type == "null_viability":
        if disp == "directional_support_with_caveats":
            trust = "supported_with_limitations"
        elif disp == "restricted_method_positive_but_primary_null_compatible":
            trust = "inconclusive"
        elif disp == "planning_diagnostic_only":
            trust = "inconclusive"

    return alignment, trust


def forbidden_action_flags(
    forbidden_actions: Sequence[str],
    violations: Sequence[str],
) -> dict[str, bool]:
    """True when the forbidden action was violated."""
    violation_set = set(violations)
    return {action: action in violation_set for action in forbidden_actions}
