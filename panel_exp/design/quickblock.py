...
class QuickBlock(Design):
    ...
    def _fit(self, X: np.ndarray, distance="euclidean") -> None:
        self.X = X
    ...
    def assign_all(
        ...
        for idx, block in enumerate(self.blocks):
            M = len(block)
            n_trt[idx] = int(max(1, np.floor(M * self.treatment_probability)))
            remainder[idx] = (
                n_trt[idx]
                - M * self.treatment_probability
                + np.random.uniform(low=0.0, high=1e-5)
            )

        N_extra = int(max(1, np.floor(N * self.treatment_probability)) - sum(n_trt))
        if N_extra > 0:
